...
author: "Samuel FANDIO NJIKAM, Samella LEUKOUO, Fanta DEMBELE, Mohamed Mehdi TRABELSSI"
output:
  html_document:
    toc: true
    toc_float: true
...
```
  ...
  Le résultat est sans appel. Les vins des tranches **25–49** et **50–99 avis** présentent des boîtes larges et des moustaches très étendues. On y trouve des notes allant de 2,7 à 4,8, signe que ces évaluations reflètent davantage l'opinion d'un cercle restreint que celle d'un consensus. À partir de **100 avis**, la dispersion se resserre nettement et se stabilise : les médianes convergent autour de 3,9 et les boîtes rétrécissent, indiquant que la note converge vers une valeur représentative.

  **Nous retenons donc un seuil de 100 avis comme filtre de fiabilité, qui sera appliqué dans toutes les analyses suivantes.** Cela représente un peu plus de la moitié du dataset; un volume largement suffisant.

  Un dernier détail intéressant : la tranche **1 000+** voit sa dispersion légèrement remonter. Ce n'est pas un manque de fiabilité, c'est l'effet inverse : les vins très médiatisés (grands crus, Champagnes de prestige) polarisent les opinions entre admirateurs et sceptiques. Ce phénomène sera à garder en tête lors des analyses sur le prix.
...
On observe une relation globalement positive entre le prix et la note des vins, ce qui confirme en partie l’idée intuitive selon laquelle les vins les plus chers sont aussi mieux notés. Cependant, cette relation reste assez modérée et surtout différente selon le type de vin.

Les vins rouges montrent la tendance la plus claire : plus le prix augmente, plus la note a tendance à augmenter également. Cela peut s’expliquer par le fait que les vins rouges couvrent une gamme de qualité et de prix très large, allant de vins simples à des grands crus très prestigieux.

Pour les vins blancs et rosés, la relation est beaucoup moins nette. Les points sont très dispersés, ce qui suggère que le prix n’est pas un bon indicateur de la qualité perçue dans ces catégories.
Les vins effervescents se situent entre les deux : on observe une tendance positive, mais elle est influencée par quelques vins très haut de gamme qui tirent la relation vers le haut.

Globalement, on remarque que le prix ne suffit pas à expliquer la qualité d’un vin. Cela montre qu’il faudra prendre en compte d’autres facteurs comme la région, le millésime ou encore les conditions climatiques pour mieux comprendre ce qui influence réellement la note.
...
Le dot plot révèle que certains producteurs surperforment structurellement leur région, avec un écart positif stable et des intervalles de confiance resserrés, signe que leur avantage n'est pas dû au hasard. Ces domaines tirent la moyenne régionale vers le haut et illustrent l'**effet domaine** : à terroir équivalent, le savoir-faire du vigneron fait une différence mesurable.

À l'inverse, quelques producteurs affichent un écart élevé mais avec de larges barres d'erreur, ce qui trahit un faible nombre de vins notés et rend leur surperformance statistiquement fragile. Le filtre à 10 vins minimum limite ce biais, mais ne l'élimine pas totalement.

La couleur par type montre que les rouges dominent ce classement, ce qui reflète à la fois leur surreprésentation dans le dataset et la plus grande amplitude de qualité au sein de cette catégorie.
...
Le nuage de points invalide l'hypothèse de départ : prix et popularité ne sont pas corrélés positivement. Le quadrant le plus dense est celui des vins **accessibles et populaires** (bas prix, nombreuses évaluations). Ce sont les vins de grande consommation, achetés et notés en masse, non pas parce qu'ils sont excellents mais parce qu'ils sont omniprésents. À l'opposé, les vins très chers se concentrent dans le quadrant **cher et confidentiel** : peu évalués, mais souvent mieux notés (teinte verte), ce qui suggère un lien entre prix élevé et qualité perçue, sans que cette qualité se traduise en volume d'évaluations.

La facette par type révèle des comportements distincts. Les rouges couvrent l'ensemble des quadrants, reflétant l'extrême diversité de cette catégorie. Les effervescents et rosés, moins représentés, se concentrent davantage dans les tranches accessibles avec un volume d'avis modéré.

Cette analyse comporte une limite importante : `NumberOfRatings` mesure la diffusion commerciale d'un vin, pas sa notoriété réelle. Un Pétrus aura toujours moins d'évaluations qu'un Bordeaux de supermarché par pure rareté, non par manque de reconnaissance. La popularité telle que mesurée ici est donc un proxy imparfait. Ce constat rejoint le seuil de fiabilité établi en Q2 : les analyses suivantes se limiteront aux vins disposant d'au moins 100 évaluations.